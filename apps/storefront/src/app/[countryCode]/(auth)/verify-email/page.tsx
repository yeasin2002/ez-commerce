...
export default function VerifyEmailPage({ params }: PageProps) {
  ...
  const onSubmit = async (data: VerifyFormData) => {
    ...
    try {
      const res = await confirmEmailVerification(data.code);
      if (res.success) {
        setIsSubmitted(true);
        reset();
        setTimeout(() => {
  ...
  const handleResend = async () => {
    if (!email) {
      setResendStatus("Please enter your email on the login page to resend.");
      return;
    }
    ...
    try {
      ...
      setResendStatus("Verification code resent! Check your email or terminal.");
    } catch {
